program dirac_exchange_dhf_mu_of_r
 ...
 !Choose Interaction
  if (dirac_interaction == "Coulomb") then
   ...
   print*, 'mu_erf =',mu_erf
   print*, 'dirac_HF_two_electron_C_Exchange_energy=', dirac_C_Exchange_Energy - dirac_HF_two_electron_c_ex_energy
   open (10, file='exchange_dhf_Z.dat',position ='append') 
   write(10,*) mu_erf, dirac_C_Exchange_Energy - dirac_HF_two_electron_c_ex_energy
 else
   print *,  'Unrecognized dirac_interaction : '//dirac_interaction
   stop 1
  endif

 !!! For 2-electrons systems 
 !For helium
